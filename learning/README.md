...
- **Self-contained learning module:** `learning/dutch-b1-learn.html` — practice UI is self-contained; Colette audio needs the local server (below)
  ...
  - Dutch TTS: **Colette** (`nl-NL-ColetteNeural`, Microsoft Edge, free) for every word, sentence, prompt and sample answer
  - Needs internet + `python learning/serve-dutch-b1.py` (open `http://127.0.0.1:8765/dutch-b1-learn.html`). `pip install edge-tts` once.
